...
import "country_codes.js" as COUNTRY

Item {
    id: root
    property alias text: displayTextItem.text
    property string placeholderText: "País"
    ...
    ComboBox {
        ...
        model: ListModel {
            ...
            Component.onCompleted: {
                for (let i = 0; i < COUNTRY.data.length; ++i)
                    theModel.append(COUNTRY.data[i])
        ...
        onCurrentIndexChanged: {
            if (currentIndex >= 0){
                root.countryCode = model.get(currentIndex).code
                root.country = model.get(currentIndex).name
            }
        }
    }
